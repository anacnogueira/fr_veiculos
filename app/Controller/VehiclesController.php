<?php
  ...
  class VehiclesController extends AppController{
     ...
     public $uses = array('Vehicle','Type','Category','Brand','Modelo','Item','ItemValue','Schedule');
      ...
      if (!empty($this->data)) {
        ...
        if($this->Vehicle->validates()){
          ...
          if ($this->Vehicle->save($data)) {
            ...
            //Salvar itens
            if(!empty($this->data['Vehicle']['allItems'])){
              $itens = explode("[item]",$this->data['Vehicle']['allItems']);

              foreach($itens as $item){
                $valores = explode("[campo]",$item);
                $data['VehicleItemValue']['vehicle_id'] = $vehicle_id;
                $data['VehicleItemValue']['item_id'] = $valores[1];
                $data['VehicleItemValue']['item_value_id'] = $valores[2];

                $this->Vehicle->VehicleItemValue->create();
                $this->Vehicle->VehicleItemValue->save($data);
              }
            }
            //Salvar opcionais
            if(!empty($this->data['item']) and is_array($this->data['item'])){
               foreach($this->data['item'] as $key => $opcional){
                  foreach($opcional as $value){
                    $data['VehicleItemValue']['vehicle_id'] = $vehicle_id;
                    $data['VehicleItemValue']['item_id'] = $key;
                    $data['VehicleItemValue']['item_value_id'] = $value;
    ...
    public function admin_edit($id = null){
       ...
       if (!empty($this->data)) {
        ...
        if($this->Vehicle->validates()){
          ...
          if ($this->Vehicle->save($data)) {
            ...
            //Apagar itens e opcionais anteriores
            $this->Vehicle->VehicleItemValue->deleteAll(array('vehicle_id'=>$vehicle_id));
           ...
           if(!empty($this->data['Vehicle']['allItems'])){
            ...
            if(!empty($this->data['item']) and is_array($this->data['item'])){
             ...
             //Excluir fotos anteriores
          $photos = $this->Vehicle->VehiclePhoto->find('all',array(
          ...
          foreach($photos as $photo){
            ...
            if(file_exists($destination."/".$photo['VehiclePhoto']['photo_ori'])){
              unlink($destination."/".$photo['VehiclePhoto']['photo_ori']);
            }
            if(file_exists($destination."/".$photo['VehiclePhoto']['photo_redim'])){
              unlink($destination."/".$photo['VehiclePhoto']['photo_redim']);
            }
          }*/

          //Excluir fotos do bd
          $this->Vehicle->VehiclePhoto->deleteAll(array('vehicle_id' => $vehicle_id));
          ...
          $this->Session->setFlash(__('O Veículo foi foi salvo.', true));
          $this->redirect(array('action'=>'index'));
      ...
      'conditions' => array('brand_id' => $this->data['Brand']['id']),
      'order'=>'Modelo.name'));
      $modelos = array(0 => "Selecione...") + $modelos;
      ...
      $opcionais  = $this->ItemValue->find('all',array(
        'recursive'=>2,
        'conditions'=>array(
        'Item.name LIKE' => 'Opcional%'
      )));

      //Carregar coleção de items
      $this->request->data['Vehicle']['allItems'] = '';

      $i = 0;
      foreach($this->data['VehicleItemValue'] as $item){

         if($item['item_id'] != 9 and $item['item_id'] != 10){
            if(isset($item['item_id']))
              $this->request->data['Vehicle']['allItems']  .= '[campo]' . $item['item_id'];
            if(isset($item['item_value_id']))
              $this->request->data['Vehicle']['allItems']  .= '[campo]' . $item['item_value_id'];


            if($i > count($item)-1)
              $this->request->data['Vehicle']['allItems'] .= "[item]";

            $i++;
         }
      }
     //Caregar tabela de itens a serem exibidos
     $tbl_itens = '';
     $itemValue = array();
     $itemValue = $this->Vehicle->VehicleItemValue->find('all',array(
       'fields'=>array('Item.*','ItemValue.*'),
        'conditions' => array('Item.name NOT LIKE' =>'Opcional%','VehicleItemValue.vehicle_id' => $id),
      ));

     foreach($itemValue as $j=>$tr){
       $tbl_itens .= "<tr>";
       $tbl_itens .= "<td><input type='checkbox' class='checkboxRemove' name='checkboxItem[]' value='" . $j ."' /></td>";
       $tbl_itens .= "<td>".$tr['Item']['name']."</td>";
       $tbl_itens .= "<td>".$tr['ItemValue']['name']."</td>";
       $tbl_itens .= "</tr>";
     }

        //Opcionais Checked
        foreach($opcionais as $idx => $opt){
          $opcionais[$idx]['ItemValue']['checked'] = false;
          foreach($this->data['VehicleItemValue'] as $itemValue){
            if($opt['ItemValue']['id'] == $itemValue['item_value_id']){
              $opcionais[$idx]['ItemValue']['checked'] = true;
            }
          }
        }


      $this->set(compact('types','categories','brands','modelos','items','opcionais','tbl_itens'));
    }
     ...
     public function admin_view($id = null){
        ...
        $this->Vehicle->bindModel(array('belongsTo'=>array(
		    ...
		    $vehicle = $this->Vehicle->read(null, $id);
      ...
      foreach($dataset as $value){
        ...
        $opts = $this->Vehicle->VehicleItemValue->find('all',array(
          'fields'=>array('Item.*','ItemValue.*'),
          'conditions' => array('Item.name LIKE' =>'Opcional%','VehicleItemValue.vehicle_id' => $id),
        ));
     ...
     public function admin_delete($id=null){
       ...
       if (!$id) {
          ...
          //Excluir fotos da pasta
          $photos = $this->Vehicle->VehiclePhoto->find('all',array(
            'conditions' => array('vehicle_id' => $id),
            'recursive' => -1
          ));
          ...
          //Excluir fotos do bd
          $this->Vehicle->VehiclePhoto->deleteAll(array('vehicle_id' => $id));

          //Ecluir dados do veículo
          $this->Vehicle->delete($id,true);

          //Excluir itens
           $this->Vehicle->VehicleItemValue->deleteAll(array('vehicle_id' => $id));
    ...
    public function view(){
      ...
      //Opcionais
      $opts = $this->Vehicle->VehicleItemValue->find('all',array(
        'fields'=>array('DISTINCT Item.*','ItemValue.*'),
        'conditions' => array('Item.name LIKE' =>'Opcional%','VehicleItemValue.vehicle_id' => $id),
      ));
      foreach($opts as $value){
        $options[$value['Item']['name']][] = $value['ItemValue']['name'];
      ...
      $title_for_layout = $vehicle['Brand']['name'] . ' ' . $vehicle['Vehicle']['name'];
      $this->set(compact('vehicle','items','options','title_for_layout'));


    }

     public function sent(){
        //Emviar form de agendamento de vista ao veículo
      if (!empty($this->data)) {
        $this->Vehicle->set($this->data);
        //Envair por e-mail
        ...
        $vehicle = $this->Vehicle->read(null,$this->data['Schedule']['vehicle_id']);

        $cc = "N";
        $body = "<h1>Novo Agendamento de veículo</h1>";
        $body .= "<p><b>".$vehicle['Brand']['name']." ".$vehicle['Vehicle']['name']." ".$vehicle['Vehicle']['manufacturing_year'].'/'.$vehicle['Vehicle']['model_year']."</b></p>";
        $body .= "<p>Nome:".$this->data['Schedule']['name']."</p>";
        $body .= "<p>E-mail:".$this->data['Schedule']['email']."</p>";
        $body .= "<p>E-Telefone:".$this->data['Schedule']['telephone']."</p>";
        $body .= "<p>Mensagem:".$this->data['Schedule']['message']."</p>";
        $body .= "<p>Data:".$this->data['Schedule']['date']."</p>";
        $body .= "<p>hora:".$this->data['Schedule']['hour']."</p>";
        if(!empty($this->data['info_add'])){
          $body .= "<ul>";
          foreach($this->data['info_add'] as $info_add){
             if($info_add == "Quero receber cópia desta proposta"){
               $cc = "S";
             }
             $body .= "<li>".$info_add."</li>";
          }
        ...
        if($cc == 'S'){
          $Email->cc($this->data['Schedule']['email']);
        }
        $Email->subject('['.Configure::read('Site.name').'] Novo Agendamento de veículo');
        ...
        $data = $this->data;
        $data['Schedule']['date'] = parent::format_date($this->data['Schedule']['date'],2);
        $data['Schedule']['options'] = implode(",",$this->data['info_add']);
        $this->Schedule->create();
        $this->Schedule->save($data);

        $this->set('vehicle',$vehicle);
        $this->set('url',$this->data['Schedule']['url']);
      }
     }
    /*---- OUTRAS FUNÇÔES -----*/
      ...
      if($this->params['form']['photo']){ //a imagem existe
       ...
       //2. Verificar tamanho do arquivo
          ...
          //if($idx <= $limite){
            ...
            $result = $this->Upload->upload($file, $destination, $name_redim, array('type' => 'resizemin', 'size' => array('370', '252'), 'output' => 'jpg'));
